...
export default function Details() {   
  ...
  <Link to={`/category/${encodeURIComponent(product.category || product.sourceCollection || 'Category')}`}>
    {product.category || product.sourceCollection || 'Category'}
  </Link>
</div>
              ...
              <div className="price-section">
                <div className="price-row">
                  ...
                  <span className="current-price">
                    <span className="price-currency">$</span>{formatPrice(configPrice || product?.price || 0)}
                  </span>
                </div>
                
                {configPrice > basePrice && (
                  <div className="base-price">Base price: ${formatPrice(basePrice)}</div>
                )}
                
                {/* Show quantity pricing if quantity > 1 */}
                {quantity > 1 && (
                  <div className="quantity-pricing">
                    <div className="unit-price">Unit price: ${formatPrice(configPrice || product?.price || 0)}</div>
                    <div className="total-price">Total: ${formatPrice(totalPrice || (configPrice || product?.price || 0) * quantity)}</div>
                  </div>
                )}
                
                <div className="price-comparison">
                  <span className="original-price">${generateOriginalPrice(configPrice || product?.price || 0)}</span>
                  <span className="savings">Save ${calculateSavings(formatPrice(configPrice || product?.price || 0), generateOriginalPrice(configPrice || product?.price || 0)).amount} ({calculateSavings(formatPrice(configPrice || product?.price || 0), generateOriginalPrice(configPrice || product?.price || 0)).percentage}%)</span>
                </div>
